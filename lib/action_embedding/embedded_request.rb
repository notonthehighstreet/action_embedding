# This is the Rails 2.3.8 Request class, which is a wrapper around a
# Rack environment hash used internally to ActionController. In the
# current application it allows us to use a common interface for Rails
# 2.2.x (which doesn't use Rask internally) and Rails 2.3.x (which
# does).
